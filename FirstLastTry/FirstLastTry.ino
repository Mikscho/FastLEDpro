...
<head>  \
    ...
    <script>  \
      ...
      var buttbright16 = document.getElementById('buttbright16'); \
      buttbright16.addEventListener('click', function(e){  \
                var xhr = new XMLHttpRequest(); \
                xhr.open('POST', '/buttbright16', true);  \
                xhr.setRequestHeader('Content-Type', 'application/json'); \
                xhr.send(JSON.stringify({ \
                    action: 'buttbright16'}));}, false); \
        } \
    </script> \
</head> \
  \
...
void startWifi() {
  ...
  server.on("/buttbright16", HTTP_POST, [](){
    switchPattern(202);
    server.send(200, "text/plain", "OK");
  });
  server.onNotFound([]() {
